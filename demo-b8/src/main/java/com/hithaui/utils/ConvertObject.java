...
public class ConvertObject {
	...
	public static Province fromProvinceDTOToProvinceDAO(ProvinceDTO provinceDTO) {
	...
	public static District fromDistrictDTOToDistrictDAO(DistrictDTO districtDTO) {
		District district = new District();
		district.setName(districtDTO.getName());
		district.setType(districtDTO.getType());
		district.setSlug(slugify(districtDTO.getName()));
		if(districtDTO.getType().compareToIgnoreCase("quan") == 0) {
			district.setNameWithType("Quận " + districtDTO.getName());
		} else if(districtDTO.getType().compareToIgnoreCase("huyen") == 0) {
			district.setNameWithType("Huyện " + districtDTO.getName());
		} else {
			district.setNameWithType("Thành phố " + districtDTO.getName());
		}
